...
for "_i" from 1 to _amount do
{
	...
	while {_relocate} do
	{
		...
		if (([_trigger, _pos] call BIS_fnc_inTrigger && !surfaceIsWater _pos && (getPos _altlogic select 2) == (getPosATL _altlogic select 2)) || _loops >= 20) then {_relocate = false;};
		_loops = _loops + 1;
	};
	//Add to array
...
for "_i" from 0 to (count _positions) - 1 do
{
	...
	[_zombie, _zombietype, _victim, _horde, 0, _positions select _i, true] exec "zombie_scripts\cly_z_init.sqs";
};
